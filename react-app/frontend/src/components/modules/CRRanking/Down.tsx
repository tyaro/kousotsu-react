...
const Dashboard = () => {
    const {data : info5M} = useSWR(
        'https://kousotsu-py.info/cryptoinfo/API/CRRank/LWR5M'
        ,{refreshInterval:30000}
    )
    const {data : info1H} = useSWR(
        'https://kousotsu-py.info/cryptoinfo/API/CRRank/LWR1H'
        ,{refreshInterval:30000}
    )
    const {data : info4H} = useSWR(
        'https://kousotsu-py.info/cryptoinfo/API/CRRank/LWR4H'
        ,{refreshInterval:30000}
    )
    const {data : info12H} = useSWR(
        'https://kousotsu-py.info/cryptoinfo/API/CRRank/LWR12H'
        ,{refreshInterval:30000}
    )
